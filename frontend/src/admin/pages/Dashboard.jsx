...
function Card({ icon: Icon, label, value, accent, to }) {
  const inner = (
  ...
  return to ? <Link to={to} data-testid={`dash-card-${label}`}>{inner}</Link> : <div data-testid={`dash-card-${label}`}>{inner}</div>;
}
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Card icon={ShoppingCart} label="Today's Orders" value={data.todays_orders} accent="bg-blue-100 text-blue-700" to="/admin/orders" />
        <Card icon={TrendingUp} label="Sales Today" value={formatBDT(data.total_sales_today)} accent="bg-aayna-mist text-aayna-rose" />
        <Card icon={Clock} label="New / Pending" value={data.new_orders} accent="bg-amber-100 text-amber-700" to="/admin/orders" />
        <Card icon={CheckCircle2} label="Confirmed" value={data.confirmed_orders} accent="bg-indigo-100 text-indigo-700" to="/admin/orders" />
        <Card icon={Truck} label="Delivered" value={data.delivered_orders} accent="bg-green-100 text-green-700" to="/admin/orders" />
        <Card icon={XCircle} label="Cancelled" value={data.cancelled_orders} accent="bg-red-100 text-red-600" to="/admin/orders" />
        <Card icon={AlertTriangle} label="Low Stock" value={data.low_stock_count} accent="bg-orange-100 text-orange-700" to="/admin/inventory" />
        <Card icon={BellOff} label="Failed Notifications" value={data.failed_notifications} accent="bg-gray-100 text-gray-600" />
      </div>
