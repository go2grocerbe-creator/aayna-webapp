...
> **Routing note:** the Kubernetes ingress sends `/api/*` to the backend and everything else to the React app. The canonical, dynamic SEO files are at `{PUBLIC_SITE_URL}/api/sitemap.xml` and `/api/robots.txt`. To serve them at the clean root paths (`/sitemap.xml`, `/robots.txt`) on the live domain, add a host/CDN rule routing those two paths to the backend. `robots.txt` advertises `{PUBLIC_SITE_URL}/sitemap.xml`.
